...
export class EnhancedProductDetectionService {
  ...
  private initializeApi(): void {
    ...
    if (apiKey && apiKey.length > 0) {
      ...
      console.log('⚠️ No Google Cloud Vision API key found, using enhanced fallback detection');
  ...
  getDetectionMethod(): string {
    return this.isApiAvailable ? 'Google Cloud Vision API' : 'Enhanced Fallback Detection';
  }
}
